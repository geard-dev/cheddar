Cheddar Core version *0.15.1* is now available from:

  <https://download.cheddar.org/cheddar-0.15.1/>
...
A number of changes to the way Cheddar Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 mCHDR per kB (0.00001 CHDR). This is to prepare for dropping the minimum transaction fee to 0.1 mCHDR per kB (0.0001 CHDR) in the next release.
